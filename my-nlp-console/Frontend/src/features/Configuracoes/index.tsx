...
import { Sparkles, ChevronRight, Settings, Palette, ShieldCheck } from 'lucide-react';
...
export default function Configuracoes() {
  ...
  // 1. Estado para guardar a role do usuário
  const [userRole, setUserRole] = useState<string>('');

  // 2. Busca o usuário ao carregar a página
  useEffect(() => {
    const userJson = localStorage.getItem('@CRM:user');
    if (userJson) {
      const user = JSON.parse(userJson);
      // Pega o nome do cargo (converte pra maiúsculo para garantir a validação)
      setUserRole(user.role ? user.role.toUpperCase() : '');
    }
  }, []);

  // 3. Define quem pode ver a sessão de acessos
  ...
  return (
    <div className="flex-1 h-full bg-white overflow-y-auto custom-scrollbar">
      ...
      <div className="max-w-5xl mx-auto px-8 py-10 space-y-12">
        ...
        {/* 4. ESCONDENDO A SEÇÃO: CONTROLE DE ACESSOS */}
        {podeGerenciarAcessos && (
          <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 pb-10 border-b border-slate-100">
            ...
            <div className="lg:col-span-4">
              <h2 className="text-lg font-black text-slate-900">Controle de Acessos</h2>
            ...
            <div className="lg:col-span-8 flex flex-col gap-4">
              <button 
                ...
                className="w-full flex items-center justify-between p-5 rounded-2xl bg-slate-50 border border-slate-200/60 hover:bg-emerald-50 hover:border-emerald-200 hover:ring-2 hover:ring-emerald-100 hover:ring-offset-2 transition-all group text-left shadow-sm"
              >
                <div className="flex items-center gap-5">
                  <div className="w-12 h-12 rounded-full bg-white shadow-sm flex items-center justify-center border border-slate-200 group-hover:border-emerald-300 group-hover:text-emerald-600 text-slate-500 transition-colors shrink-0">
                    <ShieldCheck size={20} />
                  </div>
                  <div>
                ...
                <ChevronRight size={22} className="text-slate-400 group-hover:text-emerald-500 transition-colors shrink-0" />
              </button>
            </div>
          </section>
        )}
        
        {/* SEÇÃO: SISTEMA & NOVIDADES (Visível para todos) */}
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 pb-10 border-b border-slate-100">
